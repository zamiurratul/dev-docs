...
startService() {
	...
	rm -fr /$parent_dir/$ms_name/backup/*

	#take_backup
	cp -fv /$parent_dir/$ms_name/$ms_file_name /$parent_dir/$ms_name/backup

	#remove_current
	rm -fr /$parent_dir/$ms_name/$ms_file_name
	...
	echo "start $ms_name $ms_port $ms_file_name"
	if [ $ms_name == "bll" ] 
	then 
		nohup java -Xmx1024m -jar $ms_file_name --spring.profiles.active=prod,robi >/dev/null 2>&1 &
	else
		nohup java -Xmx512m -jar $ms_file_name --spring.profiles.active=prod,robi >/dev/null 2>&1 &
	fi 
	cd ..
}
...
do
	...
	then 
		...
		echo "failed to run $ms in port $port of file $file_name"
	fi 
done
